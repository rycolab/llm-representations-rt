...
tune_val_ids = [6, 14, 20, 32, 33] #ids of documents that were in the tuning validation set
...
if __name__ == "__main__":
    ...
    full_df = pd.read_csv(fpath / "provo_word_surprisal.csv", keep_default_na=False)
    ...
    tune_df = pd.read_csv(fpath / "tuning" / f"{tune_name}.csv", keep_default_na=False)
    ...
    store_path = Path(f"{store_dir}{to_add}")
