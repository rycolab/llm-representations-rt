...
def to_tokens_and_logprobs(model, tokenizer, input, sentence_id=None, mlen=2048, edu_id=None):
    ...
    with torch.inference_mode():
        for batch in tqdm(train_loader):
            ...
            for layer_n in range(1, len_hstates):
                layer_embeddings = hstates[layer_n][0][:-1, :]
                if layer_n not in embeddings:
                    embeddings[layer_n] = []
                for input_id, embedding in zip(input_ids, layer_embeddings):
...
if __name__ == "__main__":
    ...
    for file in input_files:
        ...
        document_surprisals, embeddings = to_tokens_and_logprobs(model, tokenizer, document, mlen=max_len)
